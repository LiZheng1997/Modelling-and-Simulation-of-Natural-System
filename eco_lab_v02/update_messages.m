...
for cn=1:temp_n
    %如果这个对象是空的
    %为什么会出现对象为空?
    %是因为上面这两句 nagent=cell(1,temp_n);  nagent{cn}=agent{cn};
    %nagent=cell(1,temp_n)创建temp_n个空元素，但是只有活的才放元素，所以死的都是空元素
    %agent是死的+活的
